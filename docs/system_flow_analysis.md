...
When the system is initiated via `poetry run python -m app.cli`, the following sequence occurs:

*   **Database Preparation:** The `download_and_prepare_db()` function ensures the SQLite database (`travel2.sqlite`) is present in the `data/travel/` directory. If missing, it downloads it from a predefined URL. It also updates date fields in the database to reflect current times.
*   **Graph Visualization:** The system generates a visual representation of the LangGraph workflow and saves it as `assets/graphs/multi-agent-rag-system-graph.png`.
...
The `multi_agentic_graph` (an instance of `langgraph.graph.StateGraph`) defines the workflow. The state of this graph is managed by the `State` TypedDict (`app/core/state.py`), which includes `messages` (conversation history), `user_info` (retrieved flight details), and `dialog_state` (current assistant context).
