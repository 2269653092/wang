...
4.  **Generate and Store Embeddings:**
    *   Run the vectorizer to process data and populate Qdrant: `poetry run python vectorizer/main.py`

5.  **Run the Customer Support Chat Application:**
    *   Start the main chat application: `poetry run python -m app.cli`
...
*   **Graph Definition:** The conversation flow is defined in `app/graph.py` using LangGraph's `StateGraph`.
*   **Assistants:** Specialized assistants inherit from a base class in `app/assistants/assistant_base.py`.
*   **Tools:** Tools (functions the LLM can call) are defined in `app/tools/`.
